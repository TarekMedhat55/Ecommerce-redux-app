...
class Pagination extends Component {
    ...
    render() {
        ...
        const pages =this.getPages().map(pageNum => {
            ...
            return (<li className={buttonClass}   onClick={() => {this.onPage(pageNum)}}><button  className="page-link" key={pageNum.index} >{pageNum}</button></li>);
        });
        ...
        return (
            <nav aria-label="...">
                <ul className="pagination pg-blue justify-content-cente">
                    {prevButton}
                    {pages}
                    {nextButton}
                </ul>
